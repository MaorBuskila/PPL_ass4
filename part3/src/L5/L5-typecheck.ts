...
{   let res : boolean = false
    const te1Parents=getParentsType(te1,p)
    if (allT(isUserDefinedNameTExp, te1Parents)){
        if(isUserDefinedNameTExp(te2) ) {
            const typeNamesParents =map(element => element.typeName ,te1Parents)
            res=typeNamesParents.includes(te2.typeName)}


    }
    else {
        if (te1Parents.includes(te2)) res = true
        // te1Parents.includes(te2)? res=true : res=false
    }
    return res
}
...
export const checkEqualType = (te1: TExp, te2: TExp, exp: Exp, p: Program): Result<TExp> =>
    isAnyTExp(te2) && isAnyTExp(te2)? makeOk(te2) :
        isUserDefinedTExp(te1) && isUserDefinedTExp(te2)? makeOk(te2) :
            isRecord(te1) && isRecord(te2) ? makeOk(te2) :
                isUserDefinedNameTExp(te1) && isUserDefinedNameTExp(te2) && isSubType(te1,te2,p) ? makeOk(te2) :
                  equals(te1, te2) ? makeOk(te2) :
                      isSubType(te1,te2,p) ? makeOk(te2) :makeFailure(`Incompatible types: ${te1} and ${te2} in ${exp}`);
...
export const typeofIf = (ifExp: IfExp, tenv: TEnv, p: Program): Result<TExp> => {
    ...
    const constraint2 = bind(thenTE, (thenTE: TExp) => bind(altTE, (altTE: TExp) => checkEqualType(thenTE, altTE, ifExp, p)));
...
// TODO L51
    export const typeofLit = (exp: LitExp, _tenv: TEnv, _p: Program): Result<TExp> =>
        isNumExp(exp.val) ? makeOk(makeNumTExp()) :
            isBoolExp(exp.val) ? makeOk(makeBoolTExp()) :
                isStrExp(exp.val) ? makeOk(makeStrTExp()) :
                    isPrimOp(exp.val) ? typeofPrim(exp.val) :
                        isClosure(exp.val) ? makeOk(makeClosuerTExp()) : //check if this is corrrect, Added the method makeClosuerTExp manually
                            isSymbolSExp(exp.val) ? makeOk(makeSymbolTExp()) : //check if this is corrrect, Added the method makeSymbolTExp manually
                                isEmptySExp(exp.val) ? makeOk(makeTVar("")) : //check if this is corrrect
                                    isCompoundSExp(exp.val) ? makeOk(makePairTExp()) ://check if this is corrrect, Added the method makePairTExp manually
                                        isVoidTExp(exp.val) ? makeOk(makeVoidTExp()) ://check if this is corrrect, Added the method makePairTExp manually
                                            makeFailure(`Unknown literal type ${exp.val}`);
